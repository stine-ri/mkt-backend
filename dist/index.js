...
import providerProfile from './routes/provider/profile';
import providerRequests from './routes/provider/requests';
import providerBids from './routes/provider/bids';
import collegesRoute from './routes/provider/college';
import serviceRoutes from './routes/provider/services';
import clientRoutes from './routes/provider/client';
import { db } from '../src/drizzle/db';
import { eq, and, inArray } from 'drizzle-orm';
import * as schema from '../src/drizzle/schema';
// import { WebSocketServer } from 'ws';
// import { verify } from 'hono/jwt';
// import { registerConnection } from './lib/notification';
const app = new Hono();
app.get('/', (c) => {
...
//     registerConnection(payload.id, ws);
...
app.post('/api/service-requests', async (c) => {
    ...
    if (requestData.collegeFilter) {
        providerQuery = db
            .select()
            .from(schema.providers)
            .where(and(inArray(schema.providers.id, providerIds), eq(schema.providers.collegeId, requestData.collegeFilter)));
    }
