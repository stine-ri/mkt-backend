...
import { requests, providers } from '../../drizzle/schema';
import { eq, and, sql } from 'drizzle-orm';
import { authMiddleware, serviceProviderRoleAuth } from '../../middleware/bearAuth';
import { notifyNearbyProviders } from '../../lib/providerNotifications';
const app = new Hono();
...
app.get('/', async (c) => {
  ...
  ` : sql `true`;
    const relevantRequests = await db.query.requests.findMany({
        where: and(eq(requests.status, 'open'), distanceFilter, 
