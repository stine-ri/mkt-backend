...
import { users, providers, bids, providerServices, } from '../drizzle/schema';
import { and, eq, sql } from 'drizzle-orm';
import { sendNotification } from './notification';
// Manually extend request type to include optional latitude and longitude
export async function notifyNearbyProviders(request) {
    if (!request.id)
        return;
    ...
    if (request.isService && request.serviceId) {
        baseQuery = baseQuery.innerJoin(providerServices, and(eq(providerServices.providerId, providers.id), eq(providerServices.serviceId, request.serviceId)));
    }
